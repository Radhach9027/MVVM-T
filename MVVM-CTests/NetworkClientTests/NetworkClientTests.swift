...
class NetworkClientTests: XCTestCase {
    ...
    override func setUp() {
        super.setUp()
        requestDispatcher = NetworkRequestDispatcher(environment: NetworkEnvironment.dev, networkSession: NetworkSessionMock())
    }
    ...
    func testDataTaskSession_StubNetworkAction_ThenShouldReturnExpetcedMockModel() {
        ///ARRANGE & ACT
        let networkAction = NetworkAction(request: UserServiceEndPoint.all)
        ///ASSERT
        ...
        networkAction.fetch(in: dispatcher) { result in
            ...
            switch result {
                case  let .json(_, data):
                    ...
                    XCTAssertTrue(true)
                    if let data = data, let modelObj = try? JSONDecoder().decode([Users].self, from: data) {
